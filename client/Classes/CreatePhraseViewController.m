...
@implementation CreatePhraseViewController
...
- (void)viewWillAppear:(BOOL)animated {
	for (int i = 0; i < [languages count]; i++) {
		...
		if ([[language objectForKey:@"code"] isEqualToString:@"en"]) {
			[languagePicker selectRow:i inComponent:0 animated:NO];
		}
	}
}
...
- (IBAction)submitButtonSelected:(id)sender {
	...
	NSString *path = [NSString stringWithFormat:@"%@/api/addphrase/%@", SERVER, 
						  code];
